...
|---|---|---|---|
| dispatcher → conversion | `type_caster<T>`: `load` (Py→C++) / `cast` (C++→Py) | `include/pybind11/cast.h` (search `"class type_caster"`) | Every type that crosses the boundary implements this |
| value → holder | holder interface (`std::unique_ptr` / `shared_ptr` wrappers) | `include/pybind11/detail/...` | How a bound object owns its C++ instance |
...
| Expose a C++ class | `class_<T>` | CPython type creation (`PyType_FromSpec`); `internals` registry | (not yet traced) ◐ |
